import pandas as pd
from samples_util import samples, create_mp_cat_file
import numpy as np
...
def run_batch(input_directory, output_folder, wwtps_out_file_name,
    ...
    if not sample_exists:
        sam = samples(input_file=input_directory + uncertainty_file_name, scale_dirichlet=True)
        df = pd.DataFrame.from_dict(sam.samples)
        df.to_excel(input_directory+sample_file_name)
    ...
    if create_mp_file:
        create_mp_cat_file(input_directory + mp_settings_file_name, input_directory + mp_categories_file_name)
    ...
    for ind, row in df.iterrows():
        ...
        kwargs = {
            ...
            'mp_categories_file_name': mp_categories_file_name,
...
def model_call(ind, row, input_directory, output_folder, wwtps_out_file_name,
    ...
    dat.point_source_emissions_to_netCDF(wwtps_file, wwtps_nc_file,
                                         ...
                                         out_excel=output_folder + f"{wwtps_excel_name}_{ind}.xlsx",
...
if __name__ == '__main__':
    ...
    run_batch("D:\\Arthur\\Documents\\Thesis\\inputs\\", 
              ...
              mp_categories_file_name="final_mps5.xlsx",
